...
<div class="wrapper">
  <div class="header">
	...
	<div style="margin-left:50px; min-height:500px;">
		...
		<input type="text" name="name" value="<?php echo $line['fullname']; ?>"/>
		<?php
		...
		?>
		<input type="text" name="email" value="<?php echo $line['emailid']; ?>"/>
		<?php
		echo "</td><td style=\"width:650px\"></td></tr>";
		echo "<tr><td>Mobile No.</td><td>:</td><td>";
		?>
		<input type="text" name="mobile" value="<?php echo $line['mobileno']; ?>"/>
		<?php
		}
		?>
		<?php
		...
		if(isset($_POST['update']))
		{
		  ...
		  $mn=$_POST['mobile'];
		  $con=mysqli_connect(@"localhost","root","","dblifecare");
		  $sql="update tbladmin set `fullname`='$fn',`username`='$un',`emailid`='$em',`mobileno`='$mn' where `adminid`='$id'";
		  $result=mysqli_query($con,$sql) or die("Database table error");
