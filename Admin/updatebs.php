...
<div class="wrapper">
  <div class="header">
	...
	<div style="margin-left:50px; min-height:500px;">
		...
		$sql="select * from tblbloodstock,tblbloodgroup where `sid`='$id' and tblbloodstock.bgid=tblbloodgroup.bgid";
		$result=mysqli_query($con,$sql) or die("Database table error");
		echo "<h1> Update Record </h1>";
		echo "<table><tr><td>GroupName</td><td>:</td><td>";
		while($line=mysqli_fetch_assoc($result))
		 {
		?>
		<form id="form1" name="form1" method="post" action="" />
		<input type="text" name="groupname" value="<?php echo $line['groupname']; ?>" readonly/>
		<input type="text" name="units" value="<?php echo $line['tunits']; ?>"/>
		<?php
		...
		if(isset($_POST['back']))
		{
		  header("Location:mbloodstock.php");
		}
		if(isset($_POST['submit']))
		{
		$id=$_REQUEST['id'];
		$tun=$_POST['units'];
		$sql="update tblbloodstock set `tunits`='$tun' where `sid`='$id'";
		$result=mysqli_query($con,$sql) or die("Database table error");
		header("Location:mbloodstock.php");
