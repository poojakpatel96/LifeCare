...
<div class="wrapper">
  <div class="header">
	...
	<div style="margin-left:50px; min-height:500px;">
		...
		$sql="select * from tblbloodgroup where `bgid`='$id'";
		...
		<input type="text" name="groupname" value="<?php echo $line['groupname']; ?>" />
		<?php
		...
		while($line=mysqli_fetch_array($r))
			{
				if($line['groupname']==$gn)
					{
						echo "<h6 style=\"color:red\">This blood group already exists!</h6>";
						$flag=1;
					}
			}
		if($flag==0){
		$sql="update tblbloodgroup set `groupname`='$gn' where `bgid`='$id'";
		$result=mysqli_query($con,$sql) or die("Database table error");
		header("Location:mbloodgroup.php");
		}
		}
